...
if ($Platform -eq "windows") {
    ...
    # ── Linux / macOS ────────────────────────────────────────
    # Detect shell config — prefer zshrc (default on macOS Catalina+),
    # fall back to bashrc, then bash_profile.
    # IMPORTANT: create the file if it doesn't exist yet.
    $ShellConfig = if ($IsMacOS) {
        "$HOME/.zshrc"          # macOS default shell is zsh since Catalina
    } elseif (Test-Path "$HOME/.zshrc") {
        "$HOME/.zshrc"
    } elseif (Test-Path "$HOME/.bashrc") {
        "$HOME/.bashrc"
    } else {
        "$HOME/.bash_profile"
    }
    ...
    if (-not (Test-Path $ShellConfig)) {
        New-Item -ItemType File -Path $ShellConfig -Force | Out-Null
        Write-Host "      Created $ShellConfig" -ForegroundColor Green
    }
    ...
    if ($ShellContent -notlike "*multiexecpro*") {
        Add-Content -Path $ShellConfig -Value "`n# MultiExecPro`n$AliasLine"
